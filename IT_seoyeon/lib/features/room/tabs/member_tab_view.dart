...
class MemberTabView extends StatefulWidget {
  ...
  const MemberTabView({super.key, required this.room});
...
class _MemberTabViewState extends State<MemberTabView> {
  ...
  bool get _owner =>
      _members.any((m) => m.userId == _repo.currentUserId && m.role == MemberRole.owner);
  ...
  Future<void> _load() async {
    try{
        ...
        try {
          ...
          incomingRequests = {
            for (final r in requests) r['requester_id'].toString(): r['id'].toString(),
          };
        } catch (e) {
    ...
    await showModalBottomSheet(
      ...
      builder: (ctx) => SafeArea(
        child: ListView(
          ...
          children: [
            const Text('친구에게 방 초대', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            if (friends.isEmpty)
              const ListTile(
                title: Text('초대할 수 있는 친구가 없습니다.'),
                subtitle: Text('먼저 프로필의 친구 관리에서 친구를 추가해 주세요.'),
              ),
            ...friends.map((f) {
              final alreadyMember = memberIds.contains(f.id);
              return ListTile(
                  leading: CircleAvatar(
                    backgroundImage: f.avatarUrl == null ? null : NetworkImage(f.avatarUrl!),
                    child: f.avatarUrl == null
                        ? Text(f.nickname.isEmpty ? '?' : f.nickname[0])
                        : null,
                  ),
                  title: Text(f.nickname),
                  subtitle: Text(alreadyMember ? '이미 방 멤버입니다' : f.email),
                  trailing: alreadyMember
                      ? const Text('참여 중', style: TextStyle(color: AppColors.textSecondary))
                      : FilledButton(
                    onPressed: () async {
                      try {
                        await _repo.inviteFriendToRoom(widget.room.id, f.id);
                        if (ctx.mounted) {
                          Navigator.pop(ctx);
                          ScaffoldMessenger.of(context)
                              .showSnackBar(const SnackBar(content: Text('방 초대를 보냈습니다.')));
                        }
                      } catch (e) {
                        if (ctx.mounted) {
            ...
            children: [
              CircleAvatar(
                ...
                child: m.avatarUrl == null
                    ? Text(m.nickname.isEmpty ? '?' : m.nickname[0],
             ...
             if (isFriend)
                const Text('이미 친구입니다.', style: TextStyle(color: AppColors.textSecondary))
              else if (requestId != null)
                SizedBox(
                  width: double.infinity,
                  child: Column(
                    children: [
                      const Text('나에게 친구 요청을 보냈습니다.', style: TextStyle(color: AppColors.textSecondary)),
                        ...
                        children: [
                          Expanded(
                            child: OutlinedButton(
                              onPressed: () async {
                                try {
                                  await _repo.respondFriendRequest(requestId, false);
                          ...
                          Expanded(
                            child: FilledButton(
                              onPressed: () async {
                                try {
                                  await _repo.respondFriendRequest(requestId, true);
                                  if (ctx.mounted) Navigator.pop(ctx);
                                  if (mounted) {
                                    ScaffoldMessenger.of(context)
                                        .showSnackBar(const SnackBar(content: Text('친구가 되었습니다.')));
                                  }
                                  _load();
                                } catch (e) {
                                  if (ctx.mounted) {
        ...
        children: [
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  const Icon(Icons.person_add_alt_1, color: AppColors.room, size: 36),
                  const SizedBox(height: 8),
          ...
          Text('멤버 ${_members.length}명', style: const TextStyle(fontWeight: FontWeight.bold)),
          ..._members.map((m) => Card(
                child: ListTile(
                  ...
                  title: Text(m.nickname.isEmpty ? '프로필 미입력' : m.nickname),
                  subtitle: Text(m.role == MemberRole.owner ? '방장' : '멤버'),
                ),
              )),
        ],
      ),
